...
## W8 里程碑交付清单(压测与容错演练,7 项指标实测)

- [x] 压测脚本 `scripts/load_test.py`(stdlib 并发,零依赖;测端到端完成延迟;P50/P99/吞吐/trace 完整率/幂等拦截)
- [x] 指标1 吞吐/延迟(3 worker):提交 P50 40ms/P99 137ms;端到端 P50 5.3s/P99 6.9s;103 任务/分;8.6 子任务/秒
- [x] 指标2 水平扩展 1→3 worker:3.1→8.6 子任务/秒 = 2.77×
- [x] 指标3 幂等拦截(崩溃场景):45/165 = 27.3% 重复投递被拦
- [x] 指标4 容错零丢失:kill -9 worker 中途 → 24/24 COMPLETED
- [x] 指标5 Recall@3=1.0(W7)
- [x] 指标6 副作用幂等:85 任务 → 85 通知,0 重复
- [x] 指标7 trace 完整率 100%

**W8 发现/取舍**:
- 本机无 k6/JMeter/hey(仅 ab),自写 stdlib 压测 harness——测"提交→异步完成"的端到端语义(比纯 HTTP 压测更贴任务模型),零依赖跨机可复现。
- 扩展比 2.77×(非满 3×):单机 10 核上 3 worker+server+5 容器+压测脚本抢 CPU/DB,near-linear 符合预期,瓶颈在单机资源非架构。云上多机、分区数=worker 数时可更接近线性(留 backlog:多机压测)。
- 数字机器相关:进度报告标注环境;简历应写相对值(扩展比/拦截率/零丢失/trace 100%)+ 标注"单机 mock LLM 实测"。
- 幂等拦截率是崩溃场景值(重投触发),非稳态省 27%;稳态多数子任务只处理一次。
- EchoProcessor 300ms sleep 是压测负载基准;真实 LLM 负载延迟由模型决定(mock 下测的是调度上限)。
- 未做:云部署压测、Prometheus 指标暴露(留 W9+);真实 LLM 下的成本/延迟压测(会烧钱,按需)。

## 自检(W8 里程碑)

21. **为何测端到端完成延迟而非 HTTP 响应延迟?**
    本系统是"提交任务→异步拆解分发→worker 处理→聚合终态"的异步模型,HTTP 提交只是入口(P50 40ms)。
    用户/上游真正关心的是"任务多久跑完"(提交→COMPLETED),这才反映调度+队列+worker 的真实吞吐与
    排队延迟。只测 HTTP 会漏掉整个异步链路。故 `load_test.py` 提交后轮询到 COMPLETED 计端到端延迟。

22. **1→3 worker 扩展比 2.77×(非 3×)说明什么?**
    说明架构可水平扩展(主 topic 3 分区,3 worker 各吃 1 分区,消费组自动 rebalance),接近线性;
    差的 0.23× 来自单机资源争用(server+3 worker+5 容器+压测脚本共 10 核 CPU 与同一 MySQL/Kafka)
    和 rebalance 开销,是单机瓶颈非架构瓶颈——多机部署、分区数≥worker 数时可更接近线性。诚实结论:
    架构是水平扩展的,单机实测受限于共享资源。

23. **幂等拦截率怎么算?证明省了什么?**
    容错演练中 kill -9 一个 worker,其未提交 offset 的在途子任务经 rebalance 被其它 worker 重投处理;
    已成功处理过的那些被 `IdempotencyGuard` 拦截(trace `PROCESSED status=SKIPPED_IDEMPOTENT`)。
    拦截率 = SKIPPED / 总 PROCESSED = 45/165 = 27.3%。在真实 LLM 场景,这 27.3% 就是"崩溃重投下本会
    重复发生的 LLM 调用"被省掉——幂等把 at-least-once 投递的成本兜底成 effectively-once 的处理。

24. **压测为何用 mock LLM?**
    被测对象是自研的调度/并发/容错/幂等/可观测,不是 LLM 本身。mock LLM 确定性、零成本、可复现,
    让吞吐/扩展/零丢失这些数字只反映系统能力、不被模型延迟与费用污染(master 设计文档第 149 行既定策略)。
    真实 LLM 下的端到端延迟由模型决定,可另做但会烧钱,按需进行。
