...
> **Q: 为什么用 Kafka 不用 RocketMQ?**
> A: ① 双市场认可:Kafka 全球通用,RocketMQ 基本只在国内有认知;② 持久化日志 +
> offset 语义天然支持按位点重放,直接服务本项目"可恢复/可审计"叙事;③ 分区 + 消费组
> rebalance 原生承载 Worker 池水平扩展与上下线感知;④ KRaft 模式免 Zookeeper,部署
> 更简、架构更现代。代价:Kafka 没有原生死信队列与延迟重试——本项目在其上**自研
> 阶梯延迟重试 + DLQ(卖点5)**,既补齐能力,也能对比讲清两家 MQ 的设计取舍。
...
```
        ┌──────────── Harness(编排/管控面)────────────────┐
用户 → [API 服务] → [协调器 Coordinator] → Kafka(KRaft) → [Worker 池 ×N] ──┐
                         ↓                              │        │ Runtime
             MySQL(任务状态机)+ Redis(锁/限流)       │     (Agent 执行面)
                         ↑                              ▼        │
              [Run Trace 轨迹 → MongoDB 单机]  [上下文管理 AgentContext]
                         ↑                              ↓        │
              ┌──────────┴──────────┐       [LLM 网关] → LLM API │
...
> **技术校准记录(2026-07-11)**:原用 RocketMQ,现校准为 **Kafka(KRaft 模式)**——Kafka
> 在美国市场认可度高于 RocketMQ,RocketMQ(原属虚构项目技术栈)剔除。选型以工程最优
> 为准则,而非"是否已学过"——项目真做即真掌握;"已学过"只影响学习成本,不作否决项。
>
> **二次校准(同日,依据双市场调研 + 范围纪律)**:① Kafka 4.x 起 KRaft 免 Zookeeper,
> "Kafka+ZK" 属过时部署信号,不再捆绑;② **Zookeeper 降为 W9 可选加分**——其两个职责
> 主线有兜底(Worker 上下线感知 → Kafka 消费组 rebalance;选主 → demo 规模单协调器,
> 需要时 Redis 锁可做);③ **MongoDB 由分片集群降为单机**,分片保留为设计谈资(讲"量大
> 后按 traceId 分片"的方案,不真搭 6+ 容器);④ Kafka 无原生死信/延迟重试 → **自研补齐,
> 升级为卖点5**。主线容器:MySQL / Kafka / Redis / MongoDB 单机 / pgvector 共 5 个,
> 守住"每个组件能脱稿讲清"的原则。
...
| 数据 | 访问模式 | 选型 | 层级 | 理由 |
|---|---|---|---|---|
| **任务状态机** | 高频更新、**事务性**状态迁移、按 status/time 扫超时 | **MySQL** | 🟢 核心 | 状态机=关系型场景;ACID 保证副作用"恰好一次";索引扫超时快 |
| **任务分发队列** | 持久化日志、分区、消费者组、可重放 | **Kafka(KRaft)** | 🟢 核心 | 削峰、水平扩展消费、按 offset 重放;**死信/延迟重试自研补齐(卖点5)** |
| **热状态/锁/限流/去重键** | 毫秒读写、TTL、原子操作 | **Redis/Redisson** | 🟢 核心 | 分布式锁、令牌桶、幂等键 |
| **Run Trace** ⭐ | 深层嵌套、结构不固定、写多、按 traceId 整取回放 | **MongoDB(单机)** | 🟢 核心(降配) | 嵌套 JSON 免建表免 Join;**分片留作设计谈资**(量大后按 traceId 分片),demo 规模单机够 |
| **RAG 向量** | 近邻检索(ANN) | **pgvector** | 🟢 核心(W7) | 语义检索,一个 Postgres 扩展,成本极低(备选 Milvus/Qdrant) |
| **协调/选主/服务发现** | 强一致成员管理、watch 通知 | **Zookeeper** | 🔴 可选(W9) | 主线兜底:上下线感知 → Kafka 消费组 rebalance;选主 → 单协调器/Redis 锁。有余力才加,没时间就作为"评估过为何没上"的取舍谈资 |

**挑选标准(三问)**:主链路是否依赖它?面试 ROI(能讲多深 vs 花多少时间)?
运维成本(容器数/集成测试数)?——讲不到原理深度的组件是负资产。
...
| **任务状态机** ⭐ | pending/running/retry/failed/done + **部分成功语义** + 超时兜底 | **纯自研**(卖点2) | Java |
| **Worker 池** ⭐ | 消费 Kafka 任务、幂等去重(Redisson+MD5)、消费组 rebalance 上下线、水平扩展;跑**多角色 Agent**(抓取/分析/汇总分工) | 骨架自研;Agent 内循环用 **LangChain4j** | Java |
| **LLM 网关** ⭐ | **Redis+Lua 自实现令牌桶**、多模型路由、token 记账 | **纯自研**(卖点3) | Java |
| **Kafka 可靠性层** 🆕⭐ | Kafka 无原生死信/延迟重试 → 自研**阶梯延迟重试 topic + DLQ + 恢复重放**(对标 RocketMQ 原生语义,可对比讲) | **纯自研**(卖点5) | Java |
| **Zookeeper 协调**(可选,W9) | 协调器 Leader 选举、Worker 注册/服务发现(课程真技术);主线由消费组 rebalance + 单协调器兜底 | ZK 组件 + 集成自研 | Java |
| **Run Trace** 🆕⭐ | 每个 step / 工具调用 / 状态变更留痕(存 **MongoDB 单机**,分片为设计谈资),明确保存时机,支持回放与"闭环是否执行成功"审计 | **纯自研**(卖点4) | Java |
...
- **Kafka(KRaft)**(任务分发 + **自研延迟重试/死信层**)· Redis/Redisson(锁、限流、热状态)
- **MySQL**(任务状态机,事务)· **MongoDB 单机**(Run Trace,嵌套 JSON;分片留作设计谈资)
- **Zookeeper**(🔴 可选,W9:选主/Worker 注册,课程真技术,主线有兜底)
...
- LLM 超时/失败 → 阶梯延迟重试(**自研 retry topic**,Kafka 无原生延迟消息),超限进**自研死信 topic**(卖点5),任务标记 failed 并可人工重放
- 毒消息(反复失败)→ 死信 topic 隔离,不阻塞正常消费
- 任务超时 → 状态机定时扫描 running 超时任务,重新入队或标记失败
- 单 Agent 异常 → 隔离在子任务级,不级联拖垮整个 DAG(借鉴 RocketMQ for AI 的隔离思想,实现在 Kafka 上)
...
| 周 | 内容 | 里程碑 / 简历可写 |
|---|---|---|
| W1–2 | 骨架:API + 状态机(含部分失败语义)+ Kafka 分发 + 单 Worker(echo Agent) | 调度主链路能跑 |
| W3–4 | 可靠性:幂等、**自研 Kafka 阶梯重试/死信 topic+恢复(卖点5)**、超时兜底、多 Worker+消费组 rebalance 上下线;**Run Trace(Mongo 单机)+ 统一副作用 Policy 层** 🆕 | 幂等/容错/水平扩展/可审计 |
...
| W9 | 英文 README + 架构图 + demo GIF + 云部署(可选)+ **Zookeeper 选主/Worker 注册(可选加分)** | GitHub 门面 |
